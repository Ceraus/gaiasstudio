...
import { AlertTriangle, Keyboard, Mouse, Pencil, RotateCcw, X } from 'lucide-react'
...
function KeyboardDiagram({ highlightCombo }: { highlightCombo: string }) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3">
      {KEY_ROWS.map((row, ri) => (
        <div key={ri} className="flex flex-wrap gap-0.5 mb-0.5">
          {row.map((key) => {
            ...
            return (
              <kbd
                ...
                className={`inline-flex items-center justify-center rounded border text-[9px] font-bold uppercase transition-colors ${
                  isSpace ? 'flex-1 min-w-[80px]' : isWide ? 'min-w-[42px] px-1.5' : 'w-6'
                } h-6 ${
                  active
...
function KeyboardShortcutEditor() {
  ...
  useEffect(() => {
    ...
    function onKeyDown(e: KeyboardEvent) {
      ...
      if (!combo) {
        // Lone modifier — keep capturing, update diagram.
        const parts: string[] = []
      ...
      // Optimistic update → then broker the PATCH.
            ...
            >
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                ...
                <button
                  ...
                  onClick={() => {
                    setPendingCombo('')
                    setCapturingId(capturing ? null : id)
                  }}
...
export function MouseKeyboardPanel() {
  const dispatch = useAppDispatch()
  const { radialMenuEnabled, reducedMotion, compactDensity } = useAppSelector(
    (s) => s.preferences,
  )

  function toggle(key: 'radialMenuEnabled' | 'reducedMotion' | 'compactDensity', value: boolean) {
    dispatch(patchPreferences({ [key]: value }))
    dispatch(savePreferences({ [key]: value }))
  }
    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Tune the radial context menu and rebind any keyboard shortcut. Changes are saved
          to the Redux store, persisted to localStorage, and brokered to the user profile API.
        </p>
      </header>

      {/* ── Mouse / Radial section ── */}
      <section className={`${PANEL} space-y-4`}>
        ...
        <SettingsToggle
          ...
          onChange={(v) => toggle('radialMenuEnabled', v)}
        ...
        <div className="border-t border-slate-100 pt-4 space-y-3">
          ...
          <SettingsToggle
            ...
            onChange={(v) => toggle('compactDensity', v)}
            label="Compact density"
            hint="Tighten spacing in lists, tables, and nav items."
          />
        </div>
      </section>
